...
| US-6 | As a player I would like to ask my opponent for a rematch so that we can play against each other again. | [Winner](#winner) / [Loser](#loser) | After completing a game, the user is given the option to ask their opponent for a rematch. |
...
| US-9 | As a frequent user, I want to log in to my account so that I can access my account information. | [Login](#login) | The user can log in with their email and password. |
